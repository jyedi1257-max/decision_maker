...
| `npm run smoke` | 전체 화면을 실제로 클릭해 완주 + 스크린샷 (빌드 먼저) |
...
```
홈 ─→ 1 고민 한 문장 ─→ 2 후보 적기 ─→ 3 직감 봉인 ─→ 4 기준 (적은 순서 = 중요한 순서)
                                                          │
     회고 ←── 확정 ←── 결과 (⇄ 직접 조정해보기) ←── 내가 매긴 표 ←─ 6 평가 ←─ 5 필수조건
```

기준은 적은 순서가 곧 중요도다. 순서는 기준 줄을 **길게 눌러 끌어서** 바꾼다
(키보드는 Alt+↑/↓). 따로 '중요한 순서' 단계를 두지 않는다.
