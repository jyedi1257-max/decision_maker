...
export function robustnessSentence(sensitivity: SensitivitySummary): string {
  ...
  if (rankFlip === null) {
    return robustness === 'close'
      ...
      : '이 결과는 꽤 안정적이에요. 기준 순서를 바꿔도 잘 뒤집히지 않습니다.'
  }

  const direction = rankFlip.steps > 0 ? '더 중요하게' : '덜 중요하게'
  const amount = Math.abs(rankFlip.steps) === 1 ? '한 칸만' : `${Math.abs(rankFlip.steps)}칸`
  const target = `${ordinalMark(rankFlip.newLeaderOrdinal)}`

  if (robustness === 'close') {
    return `결과는 박빙이에요. ‘${rankFlip.criterionName}’을 지금보다 ${amount} ${direction} 보면 ${target}이 앞섭니다.`
  }
  return `이 결과는 비교적 안정적이에요. 다만 ‘${rankFlip.criterionName}’을 ${amount} ${direction} 보면 ${target}으로 바뀝니다.`
}
