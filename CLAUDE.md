...
화면에 붙이는 자리는 디자인 시스템 §5 '스티커 (화면)'에 정해져 있다. 결과·조정해보기·내가 매긴 표·확정에는 붙이지 않는다.
...
민감도는 그래프보다 문장이 먼저다 (§6.7). 숫자와 그래프는 '내가 매긴 표'와 '직접 조정해보기' 안에만 둔다.
결과 화면은 같은 말을 두 번 하지 않는다 — 그래서 '자세히'(왜 이런 결과인지) 화면을 없앴다 (2026-09-26).
